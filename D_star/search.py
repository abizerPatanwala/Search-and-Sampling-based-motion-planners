...
class DStar:
    ...
    def process_state(self):
        ...
        currNode = self.min_node()
        if currNode is None:
            return -1
        # Get neighbors of the node
        # using self.get_neighbors
        neigh = self.get_neighbors(currNode)
        k_old = self.get_k_min()
        self.delete(currNode)
        # If node k is smaller than h (RAISE)
        if k_old < currNode.h:
            for i in range(len(neigh)):
                if neigh[i].tag != "NEW" and neigh[i].h <= k_old and currNode.h > neigh[i].h + self.cost(neigh[i], currNode):
                    currNode.parent = neigh[i]
                    currNode.h = neigh[i].h + self.cost(neigh[i], currNode)
        ...
        if k_old == currNode.h:
            for i in range(len(neigh)):
                if neigh[i].tag  == "NEW" or (neigh[i].parent == currNode and neigh[i].h != currNode.h + self.cost(neigh[i], currNode)) or (neigh[i].parent != currNode and neigh[i].h > currNode.h + self.cost(neigh[i], currNode)):
                    neigh[i].parent = currNode 
                    self.insert(neigh[i], currNode.h + self.cost(neigh[i], currNode))

        # Else node k is smaller than h (RASIE)
        else:
            for i in range(len(neigh)):
                if neigh[i].tag == "NEW" or (neigh[i].parent == currNode and neigh[i].h != currNode.h + self.cost(neigh[i], currNode)):
                    neigh[i].parent = currNode
                    self.insert(neigh[i], currNode.h + self.cost(neigh[i], currNode))
                else:
                    if neigh[i].parent != currNode and neigh[i].h > currNode.h + self.cost(neigh[i], currNode):
                        self.insert(currNode, currNode.h)
                    else:
                        if neigh[i].parent != currNode and currNode.h > neigh[i].h + self.cost(neigh[i], currNode) and neigh[i].tag  == "CLOSED" and neigh[i].h > k_old:
                            self.insert(neigh[i], neigh[i].h) 
    ...
    def prepare_repair(self, node):
        ...
        neigh = self.get_neighbors(node)
        for i in range(len(neigh)):
            # If neighbor.is_dy_obs == True but neighbor.is_obs == Flase, the neighbor is a new dynamic obstacle
            if neigh[i].is_dy_obs == True and neigh[i].is_obs == False:
                # Modify the cost from this neighbor node to all this neighbor's neighbors
                # using self.modify_cost
                neigh[i].is_obs = True
                if neigh[i].tag == "CLOSED":
                    self.insert(neigh[i], math.inf)
                neigh_neigh = self.get_neighbors(neigh[i])
                for j in range(len(neigh_neigh)):
                    self.modify_cost(
                        node, neigh_neigh[j])
    ...
    def run(self):
        ...
        while(True):
            ...
            k_min = self.process_state()
            if (len(self.open) == 0) or self.start.tag == "CLOSED" or k_min == -1:
                break
