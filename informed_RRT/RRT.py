...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_centre = np.array([[(self.start.row + self.goal.row) / 2.0], [(self.start.col + self.goal.col) / 2.0]])
            ...
            while(True):
                ...
                x_u = rand_radius*math.cos(rand_angle)
                y_u = rand_radius*math.sin(rand_angle)
                xball = np.array([[x_u], [y_u]])
                # Map ball sample to the ellipsoid - x_rand
                rand_samp = np.matmul(np.matmul(C,L), xball) + x_centre   
                point = [int(rand_samp[0]), int(rand_samp[1])]
                if point[0] >= 0 and point[0]<= self.size_row and  point[1] >= 0 and point[1]<= self.size_col:
                    break    
